...
std::map<int, Eigen::Matrix<double, 7, 1>> FeatureTracker::trackFeatures(double &time, const cv::Mat &img) {
	curr_img_ = img;
	curr_time_ = time;
	...
	prev_left_features_map_.clear();
	for (size_t i = 0; i < feature_points_.ids.size(); i++) {
		prev_left_features_map_[feature_points_.ids[i]] = feature_points_.curr_features[i];
	}

	std::map<int, Eigen::Matrix<double, 7, 1>> featuresPerFrame;
	for (size_t i = 0; i < feature_points_.ids.size(); i++) {
		int feature_id = feature_points_.ids[i];
		
		double x, y, z;
		x = feature_points_.undistorted_curr_features[i].x;
		y = feature_points_.undistorted_curr_features[i].y;
		z = 1;

		double p_u, p_v;
		p_u = feature_points_.curr_features[i].x;
		p_v = feature_points_.curr_features[i].y;

		double velocity_x, velocity_y;
		velocity_x = feature_points_.velocity[i].x;
		velocity_y = feature_points_.velocity[i].y;

		Eigen::Matrix<double, 7, 1> xyz_uv_velocity;
		xyz_uv_velocity << x, y, z, p_u, p_v, velocity_x, velocity_y;
		featuresPerFrame[feature_id] = xyz_uv_velocity;
	}
...
void FeatureTracker::tracking() {
	...
	std::vector<cv::Point2f> reverse_pts = feature_points_.prev_features;
	cv::calcOpticalFlowPyrLK(curr_img_, prev_img_, feature_points_.curr_features, reverse_pts, reverse_status, error, cv::Size(21, 21), 1, 
							 cv::TermCriteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS, 30, 0.01), cv::OPTFLOW_USE_INITIAL_FLOW);
	
	for(size_t i = 0; i < status.size(); i++)
	{
		if(status[i] && reverse_status[i] && distance(feature_points_.prev_features[i], reverse_pts[i]) <= 0.5)
		{
			status[i] = 1;
		}
		else
...
void FeatureTracker::detecting() {
	...
	int num_of_features_to_track = MAX_CNT - static_cast<int>(feature_points_.curr_features.size());
	if (num_of_features_to_track > 0)
	{
		cv::goodFeaturesToTrack(curr_img_, new_features_, num_of_features_to_track, 0.01, MIN_DIST, mask_);
	}
	else {
		new_features_.clear();
	}
...
void FeatureTracker::undistortPoints() {
	...
	for (size_t i = 0; i < feature_points_.curr_features.size(); i++) {
		...
		//std::cout << projected3d.x() << " " << projected3d.y() << std::endl;
	}
}

void FeatureTracker::calcFeatureVelocity() {
...
void FeatureTracker::drawTrackImage() {
	track_img = curr_img_.clone();
	cv::cvtColor(track_img, track_img, CV_GRAY2RGB);

	for (size_t i = 0; i < feature_points_.curr_features.size(); i++) {
		double len = std::min(1.0, 1.0 * feature_points_.track_cnt[i] / 10);
		cv::circle(track_img, feature_points_.curr_features[i], 2, cv::Scalar(255 * (1 - len), 0, 255 * len), 2);
	}

	std::map<int, cv::Point2f>::iterator mapIt;
    for (size_t i = 0; i < feature_points_.ids.size(); i++)
    {
        int id = feature_points_.ids[i];
        mapIt = prev_left_features_map_.find(id);
        if(mapIt != prev_left_features_map_.end())
        {
            cv::arrowedLine(track_img, feature_points_.curr_features[i], mapIt->second, cv::Scalar(0, 255, 0), 1, 8, 0, 0.2);
        }
    }
}
...
Eigen::Vector3d FeatureTracker::liftProjective(cv::Point2f& p) {
	...
	double mx_d = (p.x / FOCAL_LENGTH_X) - (CENTER_X / FOCAL_LENGTH_X);
	double my_d = (p.y / FOCAL_LENGTH_Y) - (CENTER_Y / FOCAL_LENGTH_Y);
